...
go_UI <- function(id) {
  ...
  sidebarLayout(
    ...
    mainPanel(splitLayout(div(helpText("Show the results in this view."),
                                                           uiOutput(ns("go_plot_scroll_box")),helpText("Show output data"),uiOutput(ns('go_download')),DT::dataTableOutput(ns('go_result'))),  wellPanel(div(numericInput(ns('go_term'),'GO_term',10,1,20),uiOutput(ns("plot_setting")))),
                                                        cellWidths = c("70%","30%"))
                                  )
  )
}
